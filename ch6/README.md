...
## 6.8 매개변수 객체 만들기(skip)

## 6.9 여러 함수를 클래스로 묶기

background: 공통 데이터를 중심으로 긴밀하게 엮여 작동하는 함수 무리를 클래스 하나로 묶는다. 프로그램의 문맥을 넓게 살펴보고 정해야 함.

process:

- 1. 함수들이 공유하는 공통 데이터 레코드를 캡슐화
|__ 공통 데이터가 레코드 구조로 묶여 있지 않다면 먼저 매개 변수 객체 만들기로 데이터를 하나로 묶는 레코드 만들기

- 2. 공통 레코드를 사용하는 함수 각각을 새 클래스로 옮긴다.
|__ 공통 레코드의 멤버는 함수 호출문의 인수 목록에서 제거

- 3. 대아토룰 조작하는 로직들은 함수로 추출해서 새 클래스로 옮긴다

## 6.10 여러 함수를 변환함수로 묶기

background: 클래스로 묶는 것과 중요한 차이가 있음. 원본 데이터가 코드 안에서 갱신될때는 클래스로 묶는 편이 낫다. 변환 함수로 묶으면 새로운 레코드에 저장하므로 원본 데이터가 수정되면 일관성이 깨질 수 있다. 

이렇게 여러 함수를 한데 묶는 이유는 도출 로직이 중복되는 것을 피하기 위해서다.

process

- 1. 변환할 레코드를 입력받아서 값을 그대로 반환하는 변환 함수를 만든다
|__ 이 작업은 대체로 깊은 복사로 처리해야 함. 변환 함수가 원본 레코드를 바꾸지 않는 지 검사하는 테스트 추가 필요
